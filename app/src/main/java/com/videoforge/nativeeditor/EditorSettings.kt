    ...
    fun load(context: Context, projectId: String): EditorSettings {
        ...
        return try {
            ...
            EditorSettings(
                ...
                mosaicEnabled = j.optBoolean("mosaicEnabled", false),
                mosaicBlockSize = j.optDouble("mosaicBlockSize", 0.08).toFloat(),
                mosaicX = j.optDouble("mosaicX", 0.18).toFloat(),
                mosaicY = j.optDouble("mosaicY", 0.18).toFloat(),
                mosaicWidth = j.optDouble("mosaicWidth", 0.64).toFloat(),
                mosaicHeight = j.optDouble("mosaicHeight", 0.64).toFloat(),
                ...
                pipLayers = buildList {
                    ...
                    for (n in 0 until a.length()) {
                        ...
                        add(PipLayer(
                            ...
                            locked = q.optBoolean("locked", false)
                        ))
                    }
                }.ifEmpty {
