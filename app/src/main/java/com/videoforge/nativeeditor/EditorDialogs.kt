...
@Composable fun LayerManagerDialog(s:EditorSettings,language:AppLanguage,onChange:(EditorSettings)->Unit,onDismiss:()->Unit){val ar=language==AppLanguage.ARABIC;AlertDialog(onDismissRequest=onDismiss,title={Text(if(ar)"الطبقات" else "Layers")},text={Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(6.dp)){if(s.textLayers.isEmpty())Text(if(ar)"لا توجد طبقات نصية" else "No text layers")else s.textLayers.forEachIndexed{index,l->Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(3.dp),verticalAlignment=androidx.compose.ui.Alignment.CenterVertically){Text(l.name.ifBlank{l.text.ifBlank{"Text"}},Modifier.weight(1f),maxLines=1);IconButton(enabled=index>0,onClick={val x=s.textLayers.toMutableList();x[index]=x[index-1];x[index-1]=l;onChange(s.copy(textLayers=x))}){Text("↑")};IconButton(enabled=index<s.textLayers.lastIndex,onClick={val x=s.textLayers.toMutableList();x[index]=x[index+1];x[index+1]=l;onChange(s.copy(textLayers=x))}){Text("↓")};Switch(checked=l.visible,onCheckedChange={v->onChange(s.copy(textLayers=s.textLayers.map{if(it.id==l.id)it.copy(visible=v)else it}))});IconButton(onClick={onChange(s.copy(textLayers=s.textLayers.filterNot{it.id==l.id}))}){Text("×")}}}}}}},confirmButton={TextButton(onClick=onDismiss){Text(if(ar)"إغلاق" else "Close")}})}
