...
- SubtitleDialog يحافظ الآن على مدة الترجمة الموجودة عند تعديلها، ويستخدم نطاقًا زمنيًا دقيقًا عند تحديد الترجمة تحت رأس التشغيل.
- MarkerDialog أصبح ثنائي اللغة، مع تسمية العلامة، الانتقال إلى موضعها، حذف العلامة، وتحديث العلامة الموجودة عند إضافة علامة في نفس الموضع.
- LayerManagerDialog أضيف إليه قفل/فتح طبقة النص مع الحفاظ على الإظهار والترتيب والحذف.
- **Commit:** 2d12c9d6ecf6d339b32ba9a9eb4129d01e4da027.
- **الاختبار:** لم يُشغّل CI بعد لهذا commit؛ لا يُسجل نجاح بناء لهذه الدفعة قبل ظهور نتيجة جديدة.

## 2026-09-19 — إصلاح استرجاع إعدادات الموزايك
- تم ربط تحميل إعدادات الموزايك المحفوظة مع `EditorSettings` بحيث لا تضيع عند إغلاق المشروع وإعادة فتحه.
